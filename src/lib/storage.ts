...
function toMember(row: MemberRow): Member {
  return {
    ...
    bu: row.bu ?? undefined,
    districtInferred: row.district_inferred ?? false,
    buInferred: row.bu_inferred ?? false,
    honbuInferred: row.honbu_inferred ?? false,
    ...
    // 2026-05-13 スキップ。同じく ALTER TABLE 未実行 DB でも壊れない設計
