'use client';

// 案5: 詳細シート 上部の 大きなプライマリボタン
// メンバー詳細シートの一番上に「⏭ スキップする」を 大きく置く。
// 一番目立つ位置。「次の人へ進む」感覚で押せる。
// 行きたい★ は サブ位置に小さく。

import Link from 'next/link';
import { useState } from 'react';
import { Star, SkipForward, Phone, MapPin } from 'lucide-react';

export default function Page() {
  const [skipped, setSkipped] = useState(false);
  const [starred, setStarred] = useState(false);

  return (
    <div className="min-h-screen bg-[var(--color-bg)]">
      <header className="px-4 pt-6 pb-3 max-w-md mx-auto">
        <Link href="/mock/skip-ui" className="text-xs text-[var(--color-subtext)] underline">← 一覧へ戻る</Link>
        <h1 className="text-xl font-bold mt-2">案5 詳細シート上部 プライマリボタン</h1>
        <p className="text-xs text-[var(--color-subtext)] mt-1">
          シート開いた直後に 大きい「⏭ スキップする」が目に飛び込む。
        </p>
      </header>

      <div className="max-w-md mx-auto px-4">
        <div className="rounded-2xl border border-black/10 bg-white overflow-hidden shadow-sm">
          {/* シートトップ */}
          <div className="px-4 pt-3 pb-2">
            <div className="w-10 h-1 bg-black/15 rounded-full mx-auto" />
          </div>

          {/* ヘッダー: 名前 + ★ (★ はここでは小さい) */}
          <div className="px-4 pb-3 flex items-center gap-3">
            <div className="w-12 h-12 rounded-full bg-[#E45A5A] text-white font-bold text-base flex items-center justify-center shrink-0">
              田
            </div>
            <div className="flex-1 min-w-0">
              <div className={`font-bold text-base ${skipped ? 'text-[var(--color-subtext)] line-through' : ''}`}>
                田中 一郎
              </div>
              <div className="text-[11px] text-[var(--color-subtext)] mt-0.5">本部A ・ ヤング ・ 0.4km</div>
            </div>
            <button
              onClick={() => setStarred(!starred)}
              className={`shrink-0 inline-flex items-center justify-center w-9 h-9 rounded-full transition-colors active:scale-95 ${
                starred ? 'bg-[#FFCC00] text-white' : 'bg-[#F2F2F7] text-[#999]'
              }`}>
              <Star size={16} strokeWidth={2.2} fill={starred ? '#FFFFFF' : 'none'} />
            </button>
          </div>

          {/* 大きい スキップ プライマリボタン (案5のポイント) */}
          <div className="px-4 pb-3">
            {!skipped ? (
              <button
                onClick={() => setSkipped(true)}
                className="w-full bg-[#FF9500] text-white rounded-2xl py-3.5 px-4 flex items-center justify-center gap-2 font-bold text-[15px] shadow-[0_3px_8px_rgba(255,149,0,0.3)] active:opacity-85 transition-opacity">
                <SkipForward size={18} strokeWidth={2.5} />
                スキップする
              </button>
            ) : (
              <div className="w-full rounded-2xl border-2 border-[#FF9500] bg-[#FFF6E5] px-4 py-3 flex items-center gap-3">
                <SkipForward size={20} className="text-[#FF9500]" />
                <div className="flex-1">
                  <div className="text-[13px] font-bold text-[#7A4F00]">スキップ中</div>
                  <div className="text-[10px] text-[#7A4F00]/80">マップ非表示・スキップタブで管理</div>
                </div>
                <button onClick={() => setSkipped(false)} className="text-[12px] font-bold text-[#4A90C2] active:opacity-70 px-2 py-1">復元</button>
              </div>
            )}
          </div>

          {/* ダミーの中身 */}
          <div className="px-4 pb-5 flex flex-col gap-2 text-[12px]">
            <div className="rounded-lg bg-[#FAFAFA] p-3 flex items-center gap-2 text-[var(--color-subtext)]">
              <MapPin size={14} className="text-[#999]" /> 旭川市豊岡5条4丁目 12-3
            </div>
            <div className="rounded-lg bg-[#FAFAFA] p-3 flex items-center gap-2 text-[var(--color-subtext)]">
              <Phone size={14} className="text-[#999]" /> 090-1234-5678
            </div>
            <div className="rounded-lg bg-[#FAFAFA] p-3 text-[var(--color-subtext)]">
              最終訪問: 2026-04-12 (不在)
            </div>
          </div>
        </div>

        {/* タブ状態 */}
        <div className="mt-4 rounded-xl border border-black/10 bg-white px-3 py-3">
          <div className="text-[10px] text-[var(--color-subtext)] mb-2 font-bold tracking-wide">3 タブの状態 (参考)</div>
          <div className="flex gap-1.5 bg-[#F2F2F7] rounded-full p-1">
            <div className="flex-1 py-1.5 rounded-full bg-white shadow-sm text-center text-[12px] font-bold">いける人 <span className="ml-1 text-[10px] bg-[#111] text-white rounded-full px-1.5">{skipped ? 41 : 42}</span></div>
            <div className="flex-1 py-1.5 rounded-full text-center text-[12px] font-bold text-[var(--color-subtext)]">いけない <span className="ml-1 text-[10px] bg-black/10 text-[#666] rounded-full px-1.5">22</span></div>
            <div className="flex-1 py-1.5 rounded-full text-center text-[12px] font-bold text-[var(--color-subtext)]">スキップ <span className="ml-1 text-[10px] bg-black/10 text-[#666] rounded-full px-1.5">{skipped ? 9 : 8}</span></div>
          </div>
        </div>

        <div className="mt-4 mb-12 text-[12px] text-[var(--color-subtext)] leading-relaxed">
          <p className="mb-1"><b>👍 良いところ</b>: シート開いた瞬間 目に飛び込む。最強に目立つ。誤タップしにくい</p>
          <p className="mb-1"><b>🤔 弱いところ</b>: スキップ機能が「強すぎ」る位置。⭐ よりプライマリ扱いは違和感あるかも</p>
        </div>
      </div>
    </div>
  );
}
